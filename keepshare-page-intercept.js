...
function getPan123Token(cfg, argRaw) {
  const ctxEnv = readCtxEnv();
  const fromKeys = pickFirstValid(
    PAN123_TOKEN_KEYS.map(function (k) {
      return cfg && cfg[k];
    }).concat(
...
function buildMagnetPage(magnet, cfg, reqUrl) {
  ...
  if (show123) {
    buttons += "<a class=\"btn btn-purple\" href=\"" +
      htmlEscape(buildActionUrl(reqUrl, "123")) + "\">123云盘 · 一键导入</a>";
  }
...
function filterResolvedMagnetFiles(entries, cfg) {
  ...
  return { kept: pruned.kept, blocked: pruned.blocked };
}
...
function handle123Pan(magnet, cfg, reqUrl) {
  const argRaw = typeof $argument !== "undefined" ? String($argument || "") : "";
    ...
    try {
      ...
      if (!fileIds.length) {
        respondLocal(200, {}, htmlPage("全部被过滤", "<h1>没有可下载的文件</h1>" +
          ...
          "<p>可在模块参数调整 MAGNET_BLOCK_PATTERNS，或设置 MAGNET_FILTER=0 关闭过滤。</p>" +
          (backUrl ? "<a class=\"btn btn-secondary\" href=\"" + htmlEscape(backUrl) + "\">返回</a>" : "")));
        return;
      }
      ...
      const hint = msg.indexOf("401") !== -1 || msg.indexOf("TOKEN") !== -1
        ? "<p>Token 可能已过期，请重新登录 123 云盘并更新 PAN123_TOKEN。</p>"
        : "";
      respondLocal(200, {}, htmlPage("请求失败", "<h1>调用 123 云盘 API 失败</h1><p>" +
        htmlEscape(msg) + "</p>" + hint +
        (backUrl ? "<a class=\"btn btn-secondary\" href=\"" + htmlEscape(backUrl) + "\">返回</a>" : "")));
    }
  })();
}
...
if (!/keepshare\.(?:org|cc)/i.test(reqUrl) || !/magnet/i.test(reqUrl)) {
  ...
  if (!magnet) {
    ...
    if (action === "115") {
      ...
      handle123Pan(magnet, cfg, reqUrl);
    } else {
